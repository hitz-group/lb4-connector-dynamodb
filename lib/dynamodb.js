...
function DynamoDB(s) {
  ...
  if (
    ...
    if (process.env.DAX_ENDPOINT) {
      logger.log('debug', `Using dax service with endpoint: ${process.env.DAX_ENDPOINT}`);
      dax = new AmazonDaxClient({ endpoints: [process.env.DAX_ENDPOINT], region: awsRegion });
    }
...
DynamoDB.prototype.dropTable = require('./prototypes/dropTable.js');
DynamoDB.prototype.deleteTable = require('./prototypes/dropTable.js');
