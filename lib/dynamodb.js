...
function DynamoDB(s, dataSource) {
  ...
  if (process.env.AWS_ACCESS_KEY_ID !== undefined && process.env.AWS_SECRET_ACCESS_KEY !== undefined) {
    ...
    var awsRegion = s.region || process.env.AWS_REGION;
    AWS.config.update({
      region: awsRegion,
...
exports.initialize = function initializeSchema(ds, cb) {
  ...
  s.region = s.region || process.env.AWS_REGION || 'ap-southeast-1';
  s.accessKeyId = s.accessKeyId || process.env.AWS_ACCESS_KEY_ID || 'fake';
  s.secretAccessKey = s.secretAccessKey || process.env.AWS_SECRET_ACCESS_KEY || 'fake';

  logger.transports.console.level = ds.settings.logLevel || 'debug';
  logger.info('Initializing dynamodb adapter');
  ds.adapter = new DynamoDB(s, ds);
  ds.adapter.dataSource = ds;
 ...
 */
DynamoDB.prototype.count = function count(model, where, callback) {
  var filter = {};
  filter.where = where;
