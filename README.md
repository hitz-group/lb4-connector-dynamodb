...
Forked from [ggcasuso's](https://github.com/ggcasuso) [`loopback-connector-dynamodb`](https://github.com/ggcasuso/loopback-connector-dynamodb)
...
* The adapter first looks for environment variables `AWS_ACCESS_KEY_ID` & `AWS_SECRET_ACCESS_KEY`. If these values are found, extra settings like region, maxRetries are loaded from `dynSettings`. Example:

```javascript
    var dynSettings = { region: "us-east-1", logLevel: "info" }
```

* If environment variables cannot be located, settings are loaded by adapter from `credentials.json`. Any extra settings like log level can still be passed with `dynSettings`.
